...
void clk_3_Start(void) 
{
    ...
    clk_3_CLKEN |= clk_3_CLKEN_MASK;
	clk_3_CLKSTBY |= clk_3_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clk_3_Stop
********************************************************************************
*
* Summary:
...
void clk_3_Stop(void) 
{
    ...
    clk_3_CLKEN &= (uint8)(~clk_3_CLKEN_MASK);
	clk_3_CLKSTBY &= (uint8)(~clk_3_CLKSTBY_MASK);
}
...
{
    if ((clk_3_CLKEN & clk_3_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clk_3_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clk_3_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(clk_3_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clk_3_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clk_3_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clk_3_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clk_3_GetSourceRegister();
    uint16 oldDivider = clk_3_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clk_3_CLKEN & clk_3_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clk_3_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clk_3_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clk_3_DIV_PTR);
}


/*******************************************************************************
* Function Name: clk_3_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_3_MOD_SRC |= modeBitMask & (uint8)clk_3_MODE_MASK;
}


/*******************************************************************************
* Function Name: clk_3_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_3_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clk_3_MODE_MASK));
}


/*******************************************************************************
* Function Name: clk_3_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clk_3_MOD_SRC & (uint8)(clk_3_MODE_MASK);
}


/*******************************************************************************
* Function Name: clk_3_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clk_3_GetDividerRegister();
    uint8 oldSrc = clk_3_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clk_3_MOD_SRC |= CYCLK_SSS;
        clk_3_MOD_SRC =
            (clk_3_MOD_SRC & (uint8)(~clk_3_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clk_3_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clk_3_MOD_SRC & clk_3_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clk_3_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clk_3_PHASE = clkPhase & clk_3_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clk_3_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clk_3_PHASE & clk_3_PHASE_MASK;
}
